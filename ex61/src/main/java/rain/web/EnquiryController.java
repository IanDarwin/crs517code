...
public class EnquiryController {
	...
	public EnquiryController() {
		System.out.println("EnquiryController.EnquiryController()");
	}

	private static final String COMMAND_NAME = "enquiry";
	private static final String INPUT_FORM = "enquiry_form";
	...
	public String initialGet(ModelMap map) {
		map.addAttribute(COMMAND_NAME, new EnquiryCommand());
		return  INPUT_FORM;
	}
	...
	public String postBack(
			@Valid @ModelAttribute(COMMAND_NAME) EnquiryCommand enquiry,
			BindingResult valzResult,
			RedirectAttributes redirs
			) {
		
		if (valzResult.hasErrors()) {
			return INPUT_FORM;
		}
		
		// There would be code here to save the inquiry to db
		// and send it to marketing for processing (javamail, jms, etc)
		
		redirs.addFlashAttribute(COMMAND_NAME, enquiry);
